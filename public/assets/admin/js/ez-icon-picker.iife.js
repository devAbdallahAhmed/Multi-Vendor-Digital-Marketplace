...
var u = (i, e, o) =>
    e in i
        ? d(i, e, { enumerable: !0, configurable: !0, writable: !0, value: o })
        : (i[e] = o);
var s = (i, e, o) => u(i, typeof e != "symbol" ? e + "" : e, o);
(function () {
    "use strict";
    const i = [
        "ti ti-a-b-2",
    ...
    class e {
        constructor(t) {
            s(this, "iconPrefix", "ti ti");
            if (
                ((this.ICONS_PER_PAGE = 30),
                (this.currentPage = 0),
                (this.currentIcons = [...i]),
                (this.holder = document.querySelector(t.selector)),
                !this.holder)
            )
                throw new Error(
                    `Element with selector "${t.selector}" not found`,
                );
            this.init();
        }
        init() {
        ...
        loadInitialIcon() {
            const { icon: t } = this.holder.dataset;
            t &&
                ((this.getElement(".ez_search").value = t), this.searchIcon(t));
        }
        bindEvents() {
            (this.getElement(".ez_search").addEventListener("input", (t) =>
                this.searchIcon(t.target.value),
            ),
                this.getElement(".ez_next").addEventListener("click", () => {
                    (this.currentPage++, this.createIcons());
                }),
        ...
        createIcons() {
            const t = this.getElement(".ez_container"),
                r = this.currentPage * this.ICONS_PER_PAGE,
                a = r + this.ICONS_PER_PAGE,
                l = this.currentIcons.slice(r, a);
            (l.length === 0
                ? (t.innerHTML = `<div class="ez_no_results">
                <div class="ez_no_results_icon">😩</div>
                <div>No icons found!</div>
            </div>`)
                : (t.innerHTML = l
                      .map((n) => this.createIconElement(n))
                      .join("")),
                this.attachIconClickHandlers(),
                this.updatePaginationButtons());
        }
        createIconElement(t) {
            return `<div class="ez_icon" data-icon="${t}">
                <i class="${t}"></i>
            </div>`;
        }
        attachIconClickHandlers() {
            this.getAllElements(".ez_icon").forEach((t) => {
                t.addEventListener("click", () => {
                    (this.getAllElements(".ez_icon").forEach((r) =>
                        r.classList.remove("ez_active"),
                    ),
                        (this.getElement(".ez_search").value = t.dataset.icon),
                        t.classList.add("ez_active"));
                });
            });
        }
        updatePaginationButtons() {
            const t = this.getElement(".ez_next"),
                r = this.getElement(".ez_prev"),
                a = Math.ceil(this.currentIcons.length / this.ICONS_PER_PAGE);
            ((t.disabled = this.currentPage >= a - 1),
                (r.disabled = this.currentPage <= 0),
                (this.getElement(".ez_page-info").textContent =
                    `Page ${this.currentPage + 1} of ${a} (${this.currentIcons.length} icons)`));
        }
        searchIcon(t) {
            const r = t.toLowerCase().trim();
            if (((this.currentPage = 0), !r)) {
                ((this.currentIcons = [...i]), this.createIcons());
                return;
            }
            const a = r.split(/\s+/);
            ((this.currentIcons = i.filter((l) => {
                const n = l.replace(/-/g, " ");
                return a.every((c) => n.includes(c) || l.includes(c));
            })),
                this.createIcons());
        }
        getElement(t) {
            return this.holder.querySelector(t);
        }
        getAllElements(t) {
            return this.holder.querySelectorAll(t);
        }
    }
    ((window.EzIconPicker = e), (window.iconList = i));
})();
